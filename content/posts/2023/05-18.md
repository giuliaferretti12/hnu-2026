...
- `{% raw %} {{title}} {% endraw %}` affiche le titre du billet
...
-  `{% raw %} {{ content | safe }} {% endraw %}` insère le contenu Markdown transformé en HTML
...
N'oubliez pas d’importer ce fichier dans `global.css` en utilisant la formule `@import url('/assets/css/raleway/post.css');` si vous avez choisi de centraliser vos styles.
